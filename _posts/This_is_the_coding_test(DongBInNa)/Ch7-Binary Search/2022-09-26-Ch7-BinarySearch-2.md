...
* 전형적인 이진 탐색의 문제이자, 파라메트릭 서치(parametic search) 유형의 문제이다. 파라메트릭 서치는 최적화 문제를 **결정 문제**로 바꾸어 해결하는 기법이다. <span style="background-color:#baddfe">'원하는 조건을 만족하는 가장 알맞은 값을 찾는 문제'</span>에 주로 파라메트릭 서치를 사용한다. 
